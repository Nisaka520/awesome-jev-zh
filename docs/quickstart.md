...
- `criteria` 的 **key 是代码要用的标识符，value 是给模型的判据说明**。key 写得像代码（`billing`），value 写得像人话，两边都省不掉。
...
- **`instructions` 写成陈述句，不是问句。**「用户在抱怨」，不是「用户在抱怨吗？」。
...
把 Jev 用砸最常见的一种情况，就是直接拿 `choice` 去执行、不看 `confidence`。三段式写法可以避开它：
...
**阈值怎么定？** 拍不出来，得跑一批自己的真实数据，按置信度分桶统计准确率：
...
模型宣称是**校准**的——置信度 0.7 的那一桶，长期准确率就该在 70% 附近。**这张表一举两得：定阈值，同时验证它在你的数据上到底校不校准。** 中文数据上目前没有任何公开评测，这一步省不掉。
...
| 中文任务表现不如预期 | 训练语料构成未公开，中文无公开评测 | **自己跑一遍置信度分桶表**，英文结论搬过来风险未知 |
